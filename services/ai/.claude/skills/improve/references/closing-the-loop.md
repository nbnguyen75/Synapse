...
When the completed plans in root `plans/` reach a 20–30 plan numeric bucket (e.g. `000-020`, `021-050`) or root `plans/` exceeds 20-30 files:

1. Identify all plans marked `DONE` or `REJECTED`. Never move `TODO` or `IN PROGRESS` plans.
2. Move them into `plans/archive/XXX-YYY/` corresponding to their numeric range.
3. Update `plans/archive/README.md` with a summary of the archived batch.
4. Clean `plans/README.md` to remove rows of archived plans, replacing them with a row in the "Archived Batches" table pointing to `archive/README.md` to avoid broken relative links.
5. Alternatively, run the repo script `bun run archive:plans` to execute this automatically.

Finish with a short report: what's verified done, what was archived, what was refreshed, what's rejected, and what's executable right now.
