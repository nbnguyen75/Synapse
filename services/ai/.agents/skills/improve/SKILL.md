...
- `reconcile` → process what happened since last session: verify DONE plans, investigate BLOCKED ones, refresh drifted TODOs, retire dead findings. If completed plans reach a 20–30 plan numeric batch (e.g. `021-050`) or root `plans/` exceeds 20 files, archive completed plans into `plans/archive/XXX-YYY/` and preserve link integrity. See [references/closing-the-loop.md](references/closing-the-loop.md).
