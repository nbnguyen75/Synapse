...
export const errorHandler: ErrorHandler = (err, c) => {
  ...
  if (err instanceof HTTPException) {
    const code =
      err.status === (StatusCodes.UNAUTHORIZED as number)
        ? 'UNAUTHORIZED'
        : err.status === (StatusCodes.METHOD_NOT_ALLOWED as number)
          ? 'METHOD_NOT_ALLOWED'
          : 'VALIDATION_ERROR';

    return fail(c, code, 'Something went sideways in transit.', err.status);
  }
