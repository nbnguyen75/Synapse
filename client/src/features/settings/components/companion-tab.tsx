...
const PRESET_LABELS: Record<AiSettingsPreset, () => string> = {
  professional: () => m.settings_companion_preset_professional(),
  socratic: () => m.settings_companion_preset_socratic(),
  friendly: () => m.settings_companion_preset_friendly(),
  concise: () => m.settings_companion_preset_concise(),
  custom: () => m.settings_companion_preset_custom(),
};
...
export default function CompanionTab() {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      if (draft.preset === 'custom' && !draft.customInstructions?.trim()) {
        return;
      }
      update(draft).catch(() => {
  ...
  return (
    <div className="mt-6 space-y-6">
      <div>
        ...
        {draft.preset === 'custom' && (
          <div className="mt-4">
            <Label>{m.settings_companion_custom_instructions()}</Label>
            <Textarea
              className="mt-1"
              onChange={(e) =>
      ...
      <div>
        <Label>{m.settings_companion_bot_name()}</Label>
        <Input
          className="mt-1"
          maxLength={40}
          onChange={(e) => {
            ...
            change('botName', e.target.value);
          }}
          placeholder={DEFAULT_AI_SETTINGS.botName}
          value={draft.botName}
        />
      </div>
      ...
      <div className="flex flex-wrap items-center gap-6">
        <div>
          <Label>{m.settings_companion_response_length()}</Label>
          <NativeSelect
            className="mt-1"
            onChange={(e) =>
        ...
        <div>
          <Label>{m.settings_companion_language()}</Label>
          <NativeSelect
            className="mt-1"
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <Label>{m.settings_companion_use_emoji()}</Label>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        <Button disabled={isSaving} onClick={handleManualSave}>
          ...
          {isSaving
            ? m.settings_page_companion_saving()
        ...
        <Button
          ...
          variant="outline"
        >
          {m.settings_companion_reset_default()}
        </Button>
      </div>
    </div>
  );
